...
Design notes (maps to System Integration criterion):
  - All EnergyPlus calls are wrapped in try/except with a `crashed` flag so the
    orchestrator can detect and log a failed run instead of the process dying silently.
  - Handles (sensor/actuator lookups) are resolved once after warmup, not every
    step, to keep per-timestep latency low.
  - `on_new_state` is where you register callbacks for a fresh run; EnergyPlus
    requires a fresh state object per simulation run.
"""
...
# Edit to match the zone names actually in your .idf
...
class EnergyPlusRun:
    ...
    def __init__(self, idf_path: str, epw_path: str, output_dir: str,
        ...
        """
        on_step(snapshot) -> dict of {zone_name: {"cooling_setpoint_c": .., "heating_setpoint_c": ..}}
        Called once per zone timestep. Return {} to leave setpoints untouched
        (used for the pure "baseline" run so EnergyPlus's own schedules govern it).
        """
    ...
    def _callback(self, state):
        ...
        try:
            ...
            # --- this is the "forward injection" step: agent decision -> actuator write ---
        ...
        except Exception as e:  # noqa: BLE001 - intentional: never let EnergyPlus crash silently
            self.crashed = True
